...
export class OtpVerificationComponent implements OnInit {
  ...
  isOtpComplete(): boolean {
    ...
    // For registration and forgot password, we don't verify OTP here - just pass it to parent
    // The parent component will handle OTP verification as part of the respective process
    if (this.otpType === OtpType.Registration) {
      this.loading = false;
      this.success = 'Code validated! Completing registration...';
      this.otpVerified.emit({ verified: true, otpCode: this.otpCode });
      return;
    }

    if (this.otpType === OtpType.ForgotPassword) {
      this.loading = false;
      this.success = 'Code validated! Please set your new password.';
      this.otpVerified.emit({ verified: true, otpCode: this.otpCode });
      return;
    }

    // For other types (email verification), verify normally
